...
public class MobMember : MonoBehaviour {
	...
	void Update () {
		...
		transform.position = transform.parent.transform.position + new Vector3(Mathf.Lerp(relative.x, targetPosition.x, Time.deltaTime),Mathf.Abs(Mathf.Cos((Time.time * 5.0f) + offset)) * 0.08f* transform.parent.rigidbody.velocity.magnitude, Mathf.Lerp(relative.z, targetPosition.z, Time.deltaTime));
